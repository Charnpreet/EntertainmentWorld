...
class DataSourceProviderForTable : NSObject{
    ...
    fileprivate func loadingDataFirstTime(tablecell: TVTableViewCell, indexPath: IndexPath){
    ...
    func initlizingCollectionViewWithDataSource(tablecell: TVTableViewCell, indexPath: IndexPath){
    ...
    func loadPopularShows (pageNO: Int,  completionHandler:@escaping()->Void){
        db.loadMoviesFromDB(pageNO: pageNO, route: Routes.POPULAR_TV_SHOWS,completionHandler: { (shows : TVShowsResponse) in
            for items in shows.results {
                self.popularShows.append(items)
            }
           // self.popularShowsDataSource.popularShows = self.popularShows
            //
            //
    ...
    func loadTopratedShows(pageNO: Int, completionHandler:@escaping()->Void){
        self.db.loadMoviesFromDB(pageNO: 1, route:Routes.TOP_RATED_TV_SHOWS  ,completionHandler: { (shows: TVShowsResponse) in
            for items in shows.results {
                self.TOPRatedShows.append(items)
            }
            self.topRatedShowsDataSource.topRatedShows = self.TOPRatedShows
            self.popularShowsDataSource.popularShows = self.TOPRatedShows
    ...
    func loadOnAirShows(pageNO: Int, completionHandler:@escaping()->Void){
        self.db.loadMoviesFromDB(pageNO: 1, route:Routes.SHOWS_ON_AIR ,completionHandler: {(shows: TVShowsResponse) in
            for items in shows.results {
                self.OnAirShows.append(items)
            }
            self.onAirShowsDataSource.onAirShows = self.OnAirShows
    ...
    func loadOnAirTodayShows(pageNO: Int, completionHandler:@escaping()->Void){
        self.db.loadMoviesFromDB(pageNO: pageNO, route:Routes.SHOWS_ON_AIR_TODAY ,completionHandler: {(shows: TVShowsResponse)  in
            self.ONAirTodayShows = shows.results
            self.onAirTodayShowsDataSource.onAirTodayShows = self.ONAirTodayShows
...
extension DataSourceProviderForTable : UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        let cell = tableView.dequeueReusableCell(withIdentifier: Constants.TABLE_VIEW_CELL_IDENTIFIER, for: indexPath) as! TVTableViewCell
